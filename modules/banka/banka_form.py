...
class BankaFisiFormu(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.lookup_ana_banka.ent_display.bind("<Return>", lambda e: self.ent_tarih.focus_set())
        ...
        self.lbl_hedef_banka = tk.Label(baslik_frame, text="Hedef Banka Hesabı:")
        self.lookup_hedef_banka = LookupWidget(baslik_frame)
        self.lbl_virman_tutar = tk.Label(baslik_frame, text="Virman Tutarı:")
        self.ent_virman_tutar = tk.Entry(baslik_frame, justify='right')

        self.lookup_hedef_banka.ent_display.bind("<Return>", lambda e: self.ent_virman_tutar.focus_set())
    ...
    def ayarla_form_yapisi(self):
        ...
        if self.fis_turu == "Banka Gider Fişi":
    ...
    def toggle_hedef_banka_alani(self, goster):
        """Virman için Hedef Banka alanını gösterir/gizler."""
        if goster:
            ...
            self.lbl_hedef_banka.grid_forget()
            self.lookup_hedef_banka.grid_forget()
    ...
    def _satir_hesap_dialog_ac(self, iid):
        ...
        tur = "Gider" if self.fis_turu == "Banka Gider Fişi" else "Gelir"
    ...
    def on_satir_edit(self, iid, column, value):
        ...
        elif column in ("miktar", "birim_fiyat"):
            try:
                val = float(value)
            except (TypeError, ValueError):
                return False
            if val <= 0:
                messagebox.showwarning("Geçersiz Değer", "Miktar ve Birim Fiyat 0'dan büyük olmalıdır.", parent=self)
                return False
            satir[column] = val
        else:
            return False

        # Banka fişleri KDV'sizdir; toplamlar yine de yeniden hesaplanır
    ...
    def _satir_row_guncelle(self, iid, satir):
        ...
        self.tree_satirlar.item(iid, values=(
            ...
            self._fmt_money(satir['genel_toplam']), "❌"
        ))
    ...
    def fis_kaydet(self):
        ...
        ana_banka_id = self.lookup_ana_banka.get()
        if not ana_banka_id:
        ...
        try:
            ...
            if self.fis_id:
                ...
                mesaj = "Banka fişi başarıyla güncellendi."
            else:
                fis_kaydet(cursor, fis_baslik, fis_satirlari, kaynak_modul='Banka')
    ...
    def load_fis_data(self):
        ...
        try:
            ...
            if baslik_data['fis_turu'] in ("Bankalar Arası Virman", "Blokeyi Bankaya Aktar"):
                ...
                is_gider = baslik_data['fis_turu'] == "Banka Gider Fişi"
                for satir in satirlar:
                    satir_data = dict(zip(satir_cols, satir))
                    if satir_data['hesap_turu'] == 'Banka':
                        self.lookup_ana_banka.set(satir_data['hesap_id'])
                    else:
