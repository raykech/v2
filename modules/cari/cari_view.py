...
class CariModulu(tk.Frame):
    """Cari modülünün liste görünümü."""
    FIS_TURLERI = [
        "Alacak Dekontu",
        "Borç Dekontu",
        "Cari Ödeme",
    ...
    def create_widgets(self):
        ...
        self.cmb_cari_filtre = ttk.Combobox(filter_frame, state="readonly", width=22)
    ...
    def _load_filter_data(self):
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id, unvan FROM cariler WHERE durum=1 AND firma_id=?",
    ...
    def listele(self):
        ...
        secili_cari = self.cmb_cari_filtre.get()
        if secili_cari != "Tüm Cariler":
            cari_id = self.cari_dict.get(secili_cari)
            if cari_id:
                where_clauses.append("fs.hesap_id = ?")
                params.append(cari_id)

        # Cari hesabı içeren fişleri listele
        join_clauses = ["JOIN fis_satirlari fs ON f.id = fs.fis_id"]
        where_clauses.append("fs.hesap_turu = 'Cari'")
        ...
        try:
            ...
            messagebox.showerror("Veri Yükleme Hatası", f"Cari fişleri yüklenemedi: {e}", parent=self)
    ...
    def _update_action_buttons_state(self):
        if self.selected_fis_kaynak_modul and self.selected_fis_kaynak_modul != "Cari":
    ...
    def _kaynaga_git(self):
        ...
        if hedef_fis_id is None:
            # Bu fiş başka bir modüle ait ama türetilmiş değil (örn: Cari listesinde görünen başka modül fişi).
